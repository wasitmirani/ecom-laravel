        ...
        <tr v-for="item in products.data" :key="item.id">
          <td colspan="3">
            <div class="d-flex align-items-center">
              <div class="avatar rounded" v-if="item.product_images.length > 0">
                <div class="avatar-content">
                  <b-img-lazy
                    v-bind="mainProps"
                    :src="item.product_images[0].thumbnail"
                    rounded
                    alt=""
                  ></b-img-lazy>
                </div>
          ...
          <td>
            <p :inner-html.prop="productStatus(item.isfeatured)"></p>
          </td>
          <td>
            <p v-if="(item.category != null) & (item.cateogry != '')">
              {{ item.category.name }}
            </p>
            <span v-else>N/A</span>
          </td>
          <td>
            <p v-if="(item.subcategory != null) & (item.subcategory != '')">
          ...
          <td>
            <span>{{ $currency }}{{ item.actualPrice }}</span>
          </td>
          <td>
          ...
          <td>
            <span
              >{{ $currency }}{{ item.unitPrice - item.discount_price }}</span
            >
          </td>
          <td>

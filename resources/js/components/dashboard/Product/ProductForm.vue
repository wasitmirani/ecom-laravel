      ...
      <div class="row">
        <div class="col-md-6 col-sm-6 col-xs-12">
          ...
          >
            ...
            <span v-else>
              Your product name must be 3-150 characters long.
            </span>
          </b-form-invalid-feedback>
      ...
      <div class="row">
        ...
        <div class="col-md-6 col-sm-6 col-xs-12">
          ...
          <multiselect
            ...
            track-by="id"
          ></multiselect>
          <span class="text-danger" v-show="this.iscategory"
...
export default {
  ...
  computed: {
    validation() {
      if (this.form.name.length < 1) return null;
        ...
        return false;
      } else return this.form.name.length > 2 && this.form.name.length < 150;
    },
  },
  data() {
    return {
  ...
  methods: {
    ...
    handleRemove(file, fileList) {
      axios
        .get(
          this.$hostapi_url + "/product/delete/image?name=" + file.response,
          this.$config
        )
        .then((res) => {
          //   this.fileList.pop(file.response);
          const index = this.fileList.indexOf(file.response);
          if (index > -1) {
            this.fileList.splice(index, 1);
            this.productImagesList.splice(index, 1);
          }
    ...
    getSubCategories() {
      ...
      axios
        .get(
          this.$hostapi_url + "/product/subcategories/" + this.form.category.id,
          this.$config
        )
        .then((res) => {
    ...
    onSubmit() {
      ...
      if (this.nameValidation()) {
        ...
        fromdata.append(
          "subcategory_id",
          this.form.subcategory ? this.form.subcategory.id : 0
        );

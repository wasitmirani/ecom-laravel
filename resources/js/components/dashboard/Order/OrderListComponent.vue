    ...
    <table class="table table-hover-animation">
      ...
      <tbody>
        <tr v-for="item in orders.data" :key="item.id">
          ...
          <td>
            <span>{{ $currency }}{{ item.total_unitprice }}</span>
          </td>
          <td>
            <span>{{ $currency }}{{ item.total_discount }}</span>
          </td>
          <td>
            <span>
              {{ $currency }}
              {{ item.total_unitprice - item.total_discount }}</span
            >
          </td>
          <td>
            <p :inner-html.prop="setOrderStatus(item.status)"></p>
            <a
              v-if="item.status == 1"
              v-b-tooltip.hover
              title="Accept Order"

        ...
        <div class="row match-height">
          ...
          <div class="col-xl-3 col-md-3 col-12">
            <div class="card card-congratulation-medal">
              <div
                class="card-body"
                v-if="(this.customer[1] != null) & (this.customer[1] != '')"
              >
                <h5>Congratulations 🎉 {{ this.customer[1].name }}</h5>
                <p class="card-text font-small-3">
                  Value Buyer
                  <strong class="text-warning">{{
                    this.customer[1].name
                  }}</strong>
                </p>
                <p class="card-text font-small-3">
                  Phone <span>{{ this.customer[1].user_phone }}</span>
                </p>
                <h3 class="mb-75 mt-2 pt-50">
                  ...
                  <a href="javascript:void(0);"
                    >{{ $currency }}{{ this.customer[0].totalpurchase }}</a
                  >
                </h3>
...
export default {
  ...
  methods: {
    orderAccept(item) {
      axios
        .get(
          this.$hostapi_url + "/order/status?value=" + 2 + "&id=" + item.id,
          this.$config
        )
        .then((res) => {
          this.todayOrders();
    ...
    orderCancel(item) {
      axios
        .get(
          this.$hostapi_url + "/order/status?value=" + 0 + "&id=" + item.id,
          this.$config
        )
        .then((res) => {
          this.todayOrders();
    ...
    todayOrders() {
      let page = 1;
      axios
        .get(this.$hostapi_url + "/today-orders?page=" + page, this.$config)
        .then((res) => {
